...
// Zjisti jestli to je m3u8 nebo segment
$isM3u8Url = (stripos(parse_url($url, PHP_URL_PATH), '.m3u8') !== false) || (stripos($url, 'm3u8') !== false);

if (!$isM3u8Url) {
    // Je to segment (video/audio) - STREAMUJ HO PŘÍMO bez načítání do paměti!
    $ch = curl_init($url);
    curl_setopt_array($ch, array(
        ...
        CURLOPT_HEADERFUNCTION => function($curl, $header) {
            // Forward headers
            if (stripos($header, 'Transfer-Encoding:') === false && stripos($header, 'Content-Encoding:') === false && stripos($header, 'HTTP/') === false) {
                header($header, false);
            }
    ...
    if (!$success) {
        error_log("CURL streaming error for URL $url: " . curl_error($ch));
    }
    curl_close($ch);
    exit;
}

// Pokud je to M3U8 playlist, bufferuj ho pro zpracování
$ch = curl_init($url);
curl_setopt_array($ch, array(
    CURLOPT_FOLLOWLOCATION => true,
...
// Zkontroluj jestli je to skutečně M3U8 playlist
