...
if ($isAllowed) {
    ...
    // Výběr náhodné proxy ze seznamu
    $randomProxy = $proxies[array_rand($proxies)];
    
    // Rozdělení stringu na host, port, jméno a heslo
    list($proxyHost, $proxyPort, $proxyUser, $proxyPass) = explode(':', $randomProxy);
    
    $curlProxyAddress = $proxyHost . ':' . $proxyPort;
    $curlProxyAuth = $proxyUser . ':' . $proxyPass;
    // -------------------------
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_ENCODING => '',
        CURLOPT_PROXY => $curlProxyAddress,     // Nastavení proxy adresy
        CURLOPT_PROXYUSERPWD => $curlProxyAuth, // Nastavení proxy ověření
    ]);
    ...
    if ($response === false) {
        // Fallback: try without proxy if the Bright Data proxy fails
